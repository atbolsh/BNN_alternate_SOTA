...
from customRenorm import ReluNormalRenorm 
...
@factory
class ModifiedQuickNetFactory(ModelFactory):
    name = "quicknet"
    ...
    def residual_block(self, x: tf.Tensor) -> tf.Tensor:
        ...
        # Only difference with original is this replacement. See printerQN for an understanding of where N came from.
...
@factory
class ModifiedQuickNetSmallFactory(ModifiedQuickNetFactory):
    name = "quicknet_small"
...
@factory
class ModifiedQuickNetLargeFactory(ModifiedQuickNetFactory):
    name = "quicknet_large"
...
def ModifiedQuickNet(
    *,  # Keyword arguments only
    ...
    """Instantiates the ModifiedQuickNet architecture.
    Optionally loads weights pre-trained on ImageNet.
    ```netron
    quicknet-v1.0/quicknet.json
    ```
    ```summary
    sota.ModifiedQuickNet
    ```
    ```plot-altair
    ...
    """
    return ModifiedQuickNetFactory(
...
def ModifiedQuickNetLarge(
    *,  # Keyword arguments only
    ...
    """Instantiates the ModifiedQuickNetLarge architecture.
    Optionally loads weights pre-trained on ImageNet.
    ```netron
    quicknet-v1.0/quicknet_large.json
    ```
    ```summary
    sota.ModifiedQuickNetLarge
    ```
    ```plot-altair
    ...
    # Raises
        ValueError: in case of invalid argument for `weights`, or invalid input shape.
    """
    return ModifiedQuickNetLargeFactory(
        input_shape=input_shape,
        input_tensor=input_tensor,
        weights=weights,
        include_top=include_top,
...
def ModifiedQuickNetSmall(
    *,  # Keyword arguments only
    ...
    """Instantiates the ModifiedQuickNetSmall architecture.
    Optionally loads weights pre-trained on ImageNet.
    ```netron
    quicknet-v1.0/quicknet_small.json
    ```
    ```summary
    sota.ModifiedQuickNetSmall
    ```
    ```plot-altair
    ...
    # Raises
        ValueError: in case of invalid argument for `weights`, or invalid input shape.
    """
    return ModifiedQuickNetSmallFactory(
        input_shape=input_shape,
        input_tensor=input_tensor,
        weights=weights,
        include_top=include_top,
