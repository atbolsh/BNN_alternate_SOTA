...
@factory
class FullestQuickNetFactory(ModelFactory):
    name = "quicknet"
    ...
    def transition_block(
        ...
        # Input is after prev_blocks residual blocks, so mu = 0, sig = sqrt(prev_blocks + 1). NOT normal.
        # Though the distribution must be normal for MaxpoolRenorm to work correctly, let's see whether it works well when that's not true.
        x = tf.keras.layers.MaxPool2D(pool_size=strides, strides=strides)(x / np.sqrt(prev_blocks + 1)) # Factor out that extra sigma. And no blurpool
...
@factory
class FullestQuickNetSmallFactory(FullestQuickNetFactory):
    name = "quicknet_small"
...
@factory
class FullestQuickNetLargeFactory(FullestQuickNetFactory):
    name = "quicknet_large"
...
def FullestQuickNet(
    *,  # Keyword arguments only
    ...
    """Instantiates the FullestQuickNet architecture.
    Optionally loads weights pre-trained on ImageNet.
    ```netron
    quicknet-v1.0/quicknet.json
    ```
    ```summary
    sota.FullestQuickNet
    ```
    ```plot-altair
...
def FullestQuickNetLarge(
    *,  # Keyword arguments only
    ...
    """Instantiates the FullestQuickNetLarge architecture.
    Optionally loads weights pre-trained on ImageNet.
    ```netron
    quicknet-v1.0/quicknet_large.json
    ```
    ```summary
    sota.FullestQuickNetLarge
    ```
    ```plot-altair
    ...
    # Raises
        ValueError: in case of invalid argument for `weights`, or invalid input shape.
    """
    return FullestQuickNetLargeFactory(
        input_shape=input_shape,
        input_tensor=input_tensor,
        weights=weights,
        include_top=include_top,
...
def FullestQuickNetSmall(
    *,  # Keyword arguments only
    ...
    """Instantiates the FullestQuickNetSmall architecture.
    Optionally loads weights pre-trained on ImageNet.
    ```netron
    quicknet-v1.0/quicknet_small.json
    ```
    ```summary
    sota.FullestQuickNetSmall
    ```
    ```plot-altair
    ...
    # Raises
        ValueError: in case of invalid argument for `weights`, or invalid input shape.
    """
    return FullestQuickNetSmallFactory(
        input_shape=input_shape,
        input_tensor=input_tensor,
        weights=weights,
        include_top=include_top,
